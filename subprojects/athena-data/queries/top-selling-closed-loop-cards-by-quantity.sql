-- top 10 most selling closed cards by quantity

select gc_name, sale_qty, gc_business_model
from (select count(*) as sale_qty, TX.gc_type_code
      from transaction TX
      where tx_type = 'PURCHASE' and TX.gc_type_code in (select gc_type_code from card_type where  gc_business_model = 'close-loop')
